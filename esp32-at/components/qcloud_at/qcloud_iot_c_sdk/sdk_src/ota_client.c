...
#define OTA_RESOURCE_NAME_STR_LEN_MAX (64)
#define OTA_ONE_RESOURCE_INFO_LEN_MAX (OTA_VERSION_STR_LEN_MAX + OTA_RESOURCE_NAME_STR_LEN_MAX)
...
typedef struct {
    ...
    eOTAType current_ota_type;

    Timer report_timer;

} OTA_Struct_t;

static void _resource_callback(void *pcontext, const char *msg, uint32_t msg_len);
...
#define OTA_JSON_TYPE_VALUE_LENGTH 64
    ...
    h_ota->current_ota_type = eOTA_FW;
...
static void IOT_OTA_ResetStatus(void *handle)
{
    ...
    if (NULL != h_ota->file_name) {
        HAL_Free(h_ota->file_name);
        h_ota->file_name = NULL;
    }

    if (NULL != h_ota->file_type) {
        HAL_Free(h_ota->file_type);
        h_ota->file_type = NULL;
    }

    qcloud_otalib_md5_reinit(h_ota->md5);
}
...
#define MSG_REPORT_LEN (256)
    ...
    if (eOTA_FW == h_ota->current_ota_type) {
        ret =
            qcloud_otalib_gen_report_msg(msg_reported, MSG_REPORT_LEN, h_ota->id, h_ota->version, progress, reportType);
        if (ret != 0) {
        ...
        strcpy(topicType, "report");
    } else {
        ret = qcloud_otalib_gen_report_resource_msg(msg_reported, MSG_REPORT_LEN, h_ota->file_name, h_ota->version,
                                                    progress, reportType);
        if (ret != 0) {
    ...
    ret = qcloud_osc_report_progress(h_ota->ch_signal, msg_reported, topicType);
...
#define MSG_UPGPGRADE_LEN (256)
    ...
    int           ret, len;
    char          msg_upgrade[MSG_UPGPGRADE_LEN];
    OTA_Struct_t *h_ota         = (OTA_Struct_t *)handle;
    char          topicType[32] = {0};
    ...
    Log_i("ota type: %d, version: %s report result %d", h_ota->current_ota_type, version, reportType);
    len = strlen(version);
    ...
    if (eOTA_FW == h_ota->current_ota_type) {
        ...
        ret = qcloud_otalib_gen_report_resource_msg(msg_upgrade, MSG_UPGPGRADE_LEN, h_ota->file_name, version, 1,
                                                    reportType);
        if (ret != 0) {
            Log_e("generate resource inform message failed");
            h_ota->err = ret;
    ...
    ret = qcloud_osc_report_upgrade_result(h_ota->ch_signal, msg_upgrade, topicType);
    if (0 > ret) {
...
void *IOT_OTA_Init(const char *product_id, const char *device_name, void *ch_signal)
{
    ...
    h_ota->current_ota_type = eOTA_DEAFAULT;
    h_ota->purl             = NULL;
    h_ota->version          = NULL;
    h_ota->file_name        = NULL;
    h_ota->file_type        = NULL;
    ...
    h_ota->ch_signal = qcloud_osc_init(product_id, device_name, ch_signal, h_ota, _ota_callback, _resource_callback);
...
#define MSG_INFORM_LEN (128)
    ...
    if (eOTA_RESOURCE != h_ota->current_ota_type) {
        IOT_OTA_ResetStatus(h_ota);
    }
    ...
    ret = qcloud_osc_report_version(h_ota->ch_signal, msg_informed, "report");
    if (0 > ret) {
        Log_e("Report version failed");
        h_ota->err = ret;
        ret        = QCLOUD_ERR_FAILURE;
...
{
    return IOT_OTA_ReportUpgradeResult(handle, NULL, IOT_OTAR_UPGRADE_BEGIN);
}
...
int IOT_OTA_Ioctl(void *handle, IOT_OTA_CmdType type, void *buf, size_t buf_len)
{
    ...
    switch (type) {
        ...
        case IOT_OTAG_FILE_TYPE:
            if ((4 != buf_len) || (0 != ((unsigned long)buf & 0x3))) {
                Log_e("Invalid parameter");
                h_ota->err = IOT_OTA_ERR_INVALID_PARAM;
                return QCLOUD_ERR_FAILURE;
            } else {
                *((uint32_t *)buf) = h_ota->current_ota_type;
                break;
            }
            break;
        ...
        case IOT_OTAG_FILE_NAME:
            if (eOTA_RESOURCE == h_ota->current_ota_type) {
                strncpy(buf, h_ota->file_name, buf_len);
                ((char *)buf)[buf_len - 1] = '\0';
            } else {
                strncpy(buf, "mcu_ota_fw.bin", buf_len);
                ((char *)buf)[buf_len - 1] = '\0';
            }
            break;
...
static void _resource_callback(void *pcontext, const char *msg, uint32_t msg_len)
{
    ...
    if (h_ota->state >= IOT_OTAS_FETCHING) {
        Log_i("In downloading or downloaded state(%d)", h_ota->state);
        goto End;
    }
    ...
    h_ota->current_ota_type = eOTA_RESOURCE;

    if (qcloud_otalib_get_resource_method(msg, &json_method) != QCLOUD_RET_SUCCESS) {
    ...
    if (!strcmp(json_method, REPORT_VERSION_RSP)) {  // report version resp
        if (qcloud_otalib_get_report_version_result(msg) < QCLOUD_RET_SUCCESS) {
            Log_e("Report resource version failed!");
            // TODO: skip this error as resource channel is N.A. for now
            // h_ota->err = IOT_OTA_ERR_REPORT_VERSION;
            // h_ota->state = IOT_OTAS_FETCHED;
        } else {
        ...
        goto End;
    } else if (strcmp(json_method, UPDATE_RESOURCE) == 0) {  // update resource

        if (0 != qcloud_otalib_get_resource_params(msg, &h_ota->purl, &h_ota->version, &h_ota->file_name, h_ota->md5sum,
                                                   &h_ota->size_file)) {
            Log_e("Get resource parameter failed");
            goto End;
        }
        Log_d("res_para: file_name:%s, file_size:%d, md5:%s, version:%s, url:%s", h_ota->file_name, h_ota->size_file,
              h_ota->md5sum, h_ota->version, h_ota->purl);

        h_ota->state = IOT_OTAS_FETCHING;

    } else if (strcmp(json_method, DELETE_RESOURCE) == 0) {  // delete resource

        Log_w("delete method is NOT supported for FW!");
        if (0 != qcloud_otalib_get_resource_version(msg, &h_ota->file_name, &h_ota->version, &h_ota->file_type)) {
            Log_e("Get resource parameter failed");
            goto End;
        }

        // report success anyway
        IOT_OTA_ReportUpgradeResult(h_ota, h_ota->version, IOT_OTAR_FILE_DEL_SUCCESS);
#if 0        
		h_ota->state = IOT_OTAS_DELETING;

		if(0 != qcloud_osc_del_resource_file(h_ota->ch_signal, h_ota->file_name, h_ota->version)){
			Log_e("Delete resource file(%s) fail", h_ota->file_name);
			IOT_OTA_ReportUpgradeResult(h_ota, h_ota->version, IOT_OTAR_FILE_DEL_FAIL);
            goto End;
		} else {
			IOT_OTA_ReportUpgradeResult(h_ota, h_ota->version, IOT_OTAR_FILE_DEL_SUCCESS);
        }
...
int IOT_OTA_ReportResVersion(void *handle, uint16_t res_num, sResInfo *ver_list[])
{
    int           ret;
    int           buff_len;
    char *        msg_informed;
    ...
    POINTER_SANITY_CHECK(handle, IOT_OTA_ERR_INVALID_PARAM);
    if ((res_num > 0) && (NULL == ver_list)) {
        Log_e("Invalid argument ver_list");
        return IOT_OTA_ERR_INVALID_PARAM;
    }
    ...
    buff_len = res_num * OTA_ONE_RESOURCE_INFO_LEN_MAX + 128;
    ...
    ret = qcloud_otalib_gen_resource_ver_info(msg_informed, buff_len, res_num, ver_list);
    if (ret != 0) {
        Log_e("generate inform message failed");
        h_ota->err = ret;
        ret        = QCLOUD_ERR_FAILURE;
    ...
    ret = qcloud_osc_report_version(h_ota->ch_signal, msg_informed, "up");
    if (0 > ret) {
        Log_e("Report version failed");
        h_ota->err = ret;
        ret        = QCLOUD_ERR_FAILURE;
...
int IOT_OTA_GetResVersion(const char *json, char **filename, char **version, char **filetype)
{
    return qcloud_otalib_get_resource_version(json, filename, version, filetype);
}
