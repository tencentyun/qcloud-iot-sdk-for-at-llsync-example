...
void *IOT_Gateway_Construct(GatewayInitParam *init_param, void *mqtt_client)
{
    int          rc    = 0;
    ...
    if (init_param == NULL && mqtt_client != NULL) {
        gateway->mqtt = mqtt_client;
    } else {
    ...
    DeviceInfo *dev = IOT_MQTT_GetDeviceInfo(gateway->mqtt);
    param.product_id  = dev->product_id;
    param.device_name = dev->device_name;
    rc                = subscribe_gateway_result_topic(gateway, &param);
...
int IOT_Gateway_Subdev_Bind(void *client, GatewayParam *param, DeviceInfo *pBindSubDevInfo, long timestamp)
{
    ...
    rc = subdev_bind_hmac_sha1_cal(pBindSubDevInfo, sign, SUBDEV_BIND_SIGN_LEN, nonce, timestamp);
    if (QCLOUD_RET_SUCCESS != rc) {
        Log_e("cal sign fail: %d", rc);
    ...
    rc = gateway_publish_sync(gateway, topic, &params, &gateway->gateway_data.bind.result);
    if (QCLOUD_RET_SUCCESS != rc) {
