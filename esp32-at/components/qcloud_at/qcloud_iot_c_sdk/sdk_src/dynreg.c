...
const char *qcloud_iot_get_dynreg_host_domain(char *region)
{
    if (strncmp(region, US_EAST_HOST_REGION, MAX_SIZE_OF_PRODUCT_REGION) == 0)
        return DYN_REG_SERVER_US_EAST_URL;
    else
        return DYN_REG_SERVER_URL;
}
...
static int _post_reg_request_by_http(char *request_buf, DeviceInfo *pDevInfo)
{
    ...
    /*format URL*/
#if 1
    HAL_Snprintf(url, REG_URL_MAX_LEN, url_format, "https", qcloud_iot_get_dynreg_host_domain(pDevInfo->product_region));
