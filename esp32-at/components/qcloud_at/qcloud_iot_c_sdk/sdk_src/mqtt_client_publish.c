...
int qcloud_iot_mqtt_publish(Qcloud_IoT_Client *pClient, char *topicName, PublishParams *pParams)
{
    ...
    if (IOT_Log_Get_Level() >= eLOG_DEBUG) {
        HAL_Printf("publish packetID=%d|topicName=%s|len=%u|payload=%s\r\n", pParams->id, topicName,
                   pParams->payload_len, (char *)pParams->payload);
    } else {
        Log_i("publish packetID=%d|topicName=%s|len=%u", pParams->id, topicName, pParams->payload_len);
    }
