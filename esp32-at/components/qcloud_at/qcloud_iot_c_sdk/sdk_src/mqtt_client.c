...
const char *qcloud_iot_get_host_domain(char *region)
{
    if (strncmp(region, US_EAST_HOST_REGION, MAX_SIZE_OF_PRODUCT_REGION) == 0)
        return QCLOUD_IOT_MQTT_US_EAST_DOMAIN;
    else
        return QCLOUD_IOT_MQTT_DIRECT_DOMAIN;
}
...
int qcloud_iot_mqtt_init(Qcloud_IoT_Client *pClient, MQTTInitParams *pParams)
{
    ...
    int size;
    if (pParams->mqtt_test_server_ip)
        size = HAL_Snprintf(pClient->host_addr, HOST_STR_LENGTH, "%s", pParams->mqtt_test_server_ip);
    else
        size = HAL_Snprintf(pClient->host_addr, HOST_STR_LENGTH, "%s.%s", pParams->product_id,
                            qcloud_iot_get_host_domain(pParams->product_region));
