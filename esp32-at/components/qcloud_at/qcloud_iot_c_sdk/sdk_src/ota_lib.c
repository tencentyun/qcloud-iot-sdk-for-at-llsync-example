...
void qcloud_otalib_md5_reinit(void *pctx)
{
    iot_md5_context *ctx = (iot_md5_context *)pctx;
    ...
    switch (reportType) {
        ...
        case IOT_OTAR_MD5_NOT_MATCH:
            ret = HAL_Snprintf(buf, bufLen,
                               ...
                               "\"result_code\":\"%d\", \"result_msg\":\"MD5_not_match\"}, \"version\": \"%s\"}}",
                               reportType, version);
            break;
...
int qcloud_otalib_gen_resource_ver_info(char *buf, size_t bufLen, uint16_t res_num, sResInfo *versionList[])
{
    IOT_FUNC_ENTRY;
...
int qcloud_otalib_get_resource_method(const char *json, char **type)
{
    return _qcloud_otalib_get_firmware_varlen_para(json, METHOD_FIELD, type);
}

int qcloud_otalib_get_resource_params(const char *json, char **url, char **version, char **filename, char *md5,
                                      uint32_t *fileSize)
{
#define OTA_FILESIZE_STR_LEN (16)
    ...
    /* get file name */
    if (0 != _qcloud_otalib_get_firmware_varlen_para(json, NAME_FIELD, filename)) {
        Log_e("get value of url key failed");
        IOT_FUNC_EXIT_RC(IOT_OTA_ERR_FAIL);
    }
...
int qcloud_otalib_get_resource_version(const char *json, char **filename, char **version, char **filetype)
{
    IOT_FUNC_ENTRY;
    ...
    /* get file type */
    if (0 != _qcloud_otalib_get_firmware_varlen_para(json, FILETYPE_FIELD, filetype)) {
        Log_e("get value of version key failed");
        IOT_FUNC_EXIT_RC(IOT_OTA_ERR_FAIL);
    }
...
int qcloud_otalib_gen_report_resource_msg(char *buf, size_t bufLen, const char *file_name, const char *version,
                                          int progress, IOT_OTAReportType reportType)
{
    ...
    switch (reportType) {
        ...
        case IOT_OTAR_SPACE_NOT_ENOUGH:
            ret = HAL_Snprintf(buf, bufLen,
                               "{\"method\": \"report_result\", \"report\": {\"progress\": "
                               "{\"resource_name\":\"%s\",\"state\":\"fail\", \"result_code\":\"%d\", "
        ...
        case IOT_OTAR_MD5_NOT_MATCH:
            ret = HAL_Snprintf(buf, bufLen,
                               "{\"method\": \"report_result\", \"report\": {\"progress\": "
                               "{\"resource_name\":\"%s\",\"state\":\"fail\", \"result_code\":\"%d\", "
                               "\"result_msg\":\"MD5_not_match\"}, \"version\": \"%s\"}}",
                               file_name, reportType, version);
            break;
        case IOT_OTAR_DOWNLOAD_TIMEOUT:
        case IOT_OTAR_FILE_NOT_EXIST:
        ...
        case IOT_OTAR_FILE_DEL_SUCCESS:
            ret = HAL_Snprintf(
                buf, bufLen,
