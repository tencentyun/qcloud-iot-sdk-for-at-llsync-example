...
typedef enum {
    ...
    IOT_OTAS_DELETING,     /* deleting */
    IOT_OTAS_DELETED,      /* deleted */
...
typedef enum {
    ...
    IOT_OTAG_FETCHED_SIZE = 0, /* Size of firmware fetched */
    ...
    IOT_OTAG_FILE_NAME,        /* file name (string) */
    IOT_OTAG_FILE_TYPE,        /* file type */
...
typedef enum {
    ...
    IOT_OTAR_SPACE_NOT_ENOUGH = -6,
    IOT_OTAR_FILE_DEL_FAIL    = -7,
    IOT_OTAR_NONE             = 0,
    IOT_OTAR_DOWNLOAD_BEGIN   = 1,
    IOT_OTAR_DOWNLOADING      = 2,
    IOT_OTAR_UPGRADE_BEGIN    = 3,
    IOT_OTAR_UPGRADE_SUCCESS  = 4,
    IOT_OTAR_FILE_DEL_SUCCESS = 5,

} IOT_OTAReportType;

/* type of OTA state */
typedef enum {
    eOTA_FW = 0,   /* firmware upgrade */
    eOTA_RESOURCE, /* resource file upgrade */
    eOTA_DEAFAULT,
} eOTAType;

typedef struct _sResInfo_ {
    char *res_name;
    char *res_ver;
    char *res_type;
} sResInfo;
...
typedef enum {
    eOTA_TOPIC_SUB_SUCCESS = 0,
    eOTA_TOPIC_SUB_FAIL,
    eOTA_TOPIC_UNSUBSCRIBED,
    eRESOURCE_TOPIC_SUB_SUCCESS,
    eRESOURCE_TOPIC_SUB_FAIL,
    eRESOURCE_TOPIC_UNSUBSCRIBED,
    eRESOURCE_FILE_DEL,
    eRESOURCE_REPORT_VER_RESP,
    eSINGLE_CHANNEL_DESTROY,
} eOTA_UsrEvent;

typedef int (*OnOTAEventUsrCallback)(void *pcontext, const char *msg, uint32_t msgLen, eOTA_UsrEvent event);
...
/**
 * @brief Get resource info from json content
 *
 * @param json: resource in json format
 * @param filename:  file_name parsed from json
 * @param version:   version parsed from json
 * @param filetype:  file_type parsed from json
 *
 * @return 0 when success, or err code (<0) for failure
 */
