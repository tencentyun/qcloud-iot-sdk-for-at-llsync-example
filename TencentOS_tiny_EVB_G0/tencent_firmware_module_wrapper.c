...
int tos_tf_module_mqtt_report(const char *str)
{
    if (g_tencent_firmware_module && g_tencent_firmware_module->mqtt_report) {
		printf("mqtt report not null\n");
        return g_tencent_firmware_module->mqtt_report(str);
    }else {
		printf("mqtt report null\n");
	}
    return -1;
}
