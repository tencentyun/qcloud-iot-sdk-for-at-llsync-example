...
/* mqtt pub msg */
int esp32_tencent_firmware_module_mqtt_report(const char *payload)
{
    at_echo_t echo;

	printf("mqtt report\n");
    tos_at_echo_create(&echo, NULL, 0, "+TCREPORT:OK");

    tos_at_cmd_exec(&echo, 1000, "AT+TCREPORT=\"%s\"\r\n", payload);
    if (echo.status == AT_ECHO_STATUS_OK || echo.status == AT_ECHO_STATUS_EXPECT) {
        return 0;
    }
    return -1;
...
tencent_firmware_module_t tencent_firmware_module_esp8266 = {
	...
	.mqtt_report		= esp32_tencent_firmware_module_mqtt_report,
};
